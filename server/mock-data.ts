...
interface RegionTemplate {
  ...
  confidence: HistoryResponse['confidence'];
}

const europe: RegionTemplate = {
  summary:
    'This part of Europe sat within shifting kingdoms, trade routes, and cultural exchanges. Settlement patterns, rulers, and local economies changed frequently across centuries.',
  events: [
    {
      year: undefined,
...
const templates: Record<string, RegionTemplate> = {
  ...
  americas: {
    summary:
      'Indigenous nations, empires, and later colonial frontiers shaped this landscape. Names, borders, and languages changed dramatically over time.',
    events: [
      {
  ...
  africa: {
    summary:
      'This area participated in regional kingdoms, desert and coastal trade, and diverse agricultural systems. Historical records vary by period and location.',
    events: [
      {
  ...
  asia: {
    summary:
      'Dense agrarian societies, imperial courts, and merchant networks defined much of this region. Urban centers rose and fell with dynastic change.',
    events: [
      {
  ...
  oceania: {
    summary:
      'Island and continental communities developed sophisticated navigation, ecology management, and oral histories adapted to local environments.',
    events: [
      {
  ...
  default: {
    summary:
      'Historical knowledge for this coordinate varies. The area may have been sparsely documented, uninhabited, or known under different names in older sources.',
    events: [
      {
...
function regionFor(lat: number, lng: number): RegionTemplate {
  if (lat >= 35 && lat <= 72 && lng >= -25 && lng <= 45) return templates.europe;
  if (lat >= -56 && lat <= 72 && lng >= -170 && lng <= -30) return templates.americas;
  if (lat >= -35 && lat <= 38 && lng >= -20 && lng <= 55) return templates.africa;
  if (lat >= -10 && lat <= 55 && lng >= 45 && lng <= 150) return templates.asia;
  if (lat >= -50 && lat <= 10 && lng >= 110 && lng <= 180) return templates.oceania;
